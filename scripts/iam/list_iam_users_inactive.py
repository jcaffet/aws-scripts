#!/usr/bin/python
# This script displays a list of unsed users
...
def main(argv):
   ...
   for user in all_users:
      ...
      most_recent_accesskey_age = get_older_accesskey_usage(user)  
      ...
      if    (last_password_login_age > MAX_ACTIVITY_AGE or last_password_login_age == -1) \
        and (most_recent_accesskey_age > MAX_ACTIVITY_AGE or most_recent_accesskey_age == -1) \
        and (user_age > MAX_ACTIVITY_AGE):
        print("%s;%s;%s;%s" % (user.user_name, user_age, last_password_login_age,most_recent_accesskey_age))


def get_older_accesskey_usage(user):
